...
#import "XHPathUtility.h"
#import "NSString+Common.h"

@implementation XHPathUtility
...
+ (NSString *)getFileDocumentPath:(NSString *)fileName
{
    ...
    NSString *documentDirectory = [XHPathUtility getDocumentPath];
...
+ (NSString *)getFileCachePath:(NSString *)fileName
{
    ...
    NSString *cacheDirectory = [XHPathUtility getCachePath];
...
+ (BOOL)isExistFileInDocument:(NSString *)fileName
{
    ...
    NSString *filePath = [XHPathUtility getFileDocumentPath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
+ (BOOL)isExistFileInCache:(NSString *)fileName
{
    ...
    NSString *filePath = [XHPathUtility getFileCachePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
+ (BOOL)removeFolderInDocumet:(NSString *)aFolderNameInDoc
{
    ...
    NSString *filePath = [XHPathUtility getFileDocumentPath:aFolderNameInDoc];
    if (nil == filePath)
    {
        return YES;
    }
...
+ (BOOL)removeFolderInCahe:(NSString *)aFolderNameInCahe
{
    ...
    if (![XHPathUtility isExistFileInCache:aFolderNameInCahe]) {
        return YES;
    }
    
    NSString *filePath = [XHPathUtility getFileCachePath:aFolderNameInCahe];
    if (nil == filePath)
    {
        return YES;
    }
...
+ (BOOL)isExistFileInResource:(NSString *)fileName
{
    ...
    NSString *filePath = [XHPathUtility getFileResourcePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
+ (BOOL)copyResourceFileToDocumentPath:(NSString *)resourceName
{
    ...
    //获取资源文件的存放目录进行
    NSString *resourcePath = [XHPathUtility getFileResourcePath:resourceName];
    NSString *documentPath = [XHPathUtility getFileDocumentPath:resourceName];
    ...
    if ([XHPathUtility isExistFile:documentPath])
    {
        // 如果文件已存在， 那么先删除原来的
        [XHPathUtility deleteFileAtPath:documentPath];
    }
...
+ (BOOL)createDirectoryAtDocument:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [XHPathUtility getFileDocumentPath:dirName];
...
+ (BOOL)createDirectoryAtCache:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [XHPathUtility getFileCachePath:dirName];
    if ([fileManager fileExistsAtPath:dirPath])
    {
        return YES;
    }
...
+ (BOOL)createDirectoryAtTemporary:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *tempPath = [XHPathUtility getTemporaryPath];
...
+ (NSString *)reCorrentPathWithPath:(NSString *)path
{
    ...
    NSString *docPath = [XHPathUtility getDocumentPath];
    ...
    if (range.length <= 0)
    {
        ...
        if (docRange.length > 0)
        {
            ...
            NSString *newPath = [XHPathUtility getFileDocumentPath:relPath];
            return newPath;
        }
    }
    return path;
...
{
    return [XHPathUtility getTemporaryPath];
}
