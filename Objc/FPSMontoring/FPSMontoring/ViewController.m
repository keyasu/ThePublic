...
//  FPSMontoring
//
//  Created by douhuo on 2021/4/19.
//
...
- (void)fpsDisplayLinkAction:(CADisplayLink *)link {
    ...
    _lastTime = link.timestamp;
    _fps = _count / delta;
    NSLog(@"count = %lu, delta = %f,_lastTime = %f, _fps = %.0f",(unsigned long)_count, delta, _lastTime, _fps);
    self.count = 0;
...
- (void)displayFps: (CADisplayLink *)fpsDisplay {
    ...
    if (threshold >= 1.0) {
        ...
        _fpsLbe.text = [NSString stringWithFormat:@"FPS:%.0f",_fps];
        self.count = 0;
        NSLog(@"count = %lu,_lastTime = %f, _fps = %.0f",(unsigned long)_count, _lastTime, _fps);
    }
}
